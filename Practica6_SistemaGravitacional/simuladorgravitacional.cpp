...
//     return true;
// }
// bool SimuladorGravitacional::guardarTrayectoriasCSV(const QString& rutaArchivo) const
// {
//     QFile archivo(rutaArchivo);
...
//     for (int i = 0; i < m_cuerpos.size(); ++i) {
//         encabezados << QString("%1_x").arg(m_cuerpos[i]->getNombre());
//         encabezados << QString("%1_y").arg(m_cuerpos[i]->getNombre());
//     }
//     stream << encabezados.join(",") << "\n";

//     // Escribir datos de trayectorias
//     for (const auto& estado : m_historiaTrayectorias) {
//         QStringList linea;
//         for (const QPointF& posicion : estado) {
//             linea << QString::number(posicion.x(), 'f', 6);
//             linea << QString::number(posicion.y(), 'f', 6);
//         }
//         stream << linea.join(",") << "\n";
//     }

//     archivo.close();
//     qDebug() << "Trayectorias guardadas en" << rutaArchivo;
//     return true;
// }
// bool SimuladorGravitacional::guardarEstadoActualCSV(const QString& rutaArchivo) const
// {
//     QFile archivo(rutaArchivo);
//     if (!archivo.open(QIODevice::WriteOnly | QIODevice::Text)) {
//         return false;
...
//     // Escribir encabezado
//     stream << "nombre,x,y,masa,radio,vx,vy\n";

//     // Escribir datos de cada cuerpo
//     for (const CuerpoEstelar* cuerpo : m_cuerpos) {
//         stream << formatearEstadoCuerpo(cuerpo) << "\n";
//     }

//     archivo.close();
//     qDebug() << "Estado actual guardado en" << rutaArchivo;
//     return true;
// }
...
// }
// QString SimuladorGravitacional::formatearEstadoCuerpo(const CuerpoEstelar* cuerpo) const
// {
//     return QString("%1,%2,%3,%4,%5,%6,%7")
//     .arg(cuerpo->getNombre())
//         .arg(cuerpo->getPosX(), 0, 'f', 6)
//         .arg(cuerpo->getPosY(), 0, 'f', 6)
//         .arg(cuerpo->getMasa(), 0, 'f', 6)
//         .arg(cuerpo->getRadio(), 0, 'f', 6)
//         .arg(cuerpo->getVelX(), 0, 'f', 6)
//         .arg(cuerpo->getVelY(), 0, 'f', 6);
// }
...
// NUEVA FUNCIONALIDAD: Resetear a posiciones originales
...
void SimuladorGravitacional::actualizarSimulacion()
{
    ...
    try {
        ...
        // NUEVA: Detectar colisiones si está habilitado
...
// NUEVA FUNCIONALIDAD: Detección de colisiones
...
// NUEVA FUNCIONALIDAD: Carga automática CSV/TXT
...
// NUEVA FUNCIONALIDAD: Cargar archivos TXT
bool SimuladorGravitacional::cargarDesdeTXT(const QString& rutaArchivo)
{
    ...
    while (!stream.atEnd()) {
        ...
        // Para archivos TXT, asumir que contienen solo datos numéricos
        QStringList campos = parsearLineaTXT(linea);

        // Formato esperado para TXT: nombre x y masa radio vx vy
        if (campos.size() < 7) {
            // Si hay menos de 7 campos, podría ser un archivo de trayectorias
            // No crear cuerpos en este caso
            continue;
        }
...
// Funciones existentes (CSV) mantenidas igual...
bool SimuladorGravitacional::cargarDesdeCSV(const QString& rutaArchivo)
{
    QFile archivo(rutaArchivo);
    if (!archivo.open(QIODevice::ReadOnly | QIODevice::Text)) {
