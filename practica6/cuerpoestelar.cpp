    ...
    m_enColision(false), m_mostrarTrayectoria(true)
{
    ...
    // Guardar posición original automáticamente
...
{
    // Rectángulo que contiene el cuerpo y su trayectoria con margen dinámico
...
void CuerpoEstelar::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    // Dibujar trayectoria completa solo si está habilitada
    if (m_mostrarTrayectoria && m_trayectoria.size() > 1) {
        ...
        // Dibujar trayectoria relativa a la posición actual del objeto
        ...
        // Dibujar puntos en la trayectoria para mejor visualización
    ...
    // Color especial si está en colisión
    ...
    // Efecto de colisión con anillo pulsante
    if (m_enColision) {
    ...
    // Mostrar información adicional si hay colisión
    if (m_enColision) {
...
void CuerpoEstelar::actualizarPosicion(double dt)
{
    ...
    setPos(m_x, m_y);
    // Solo agregar a trayectoria si está habilitada
...
void CuerpoEstelar::calcularFuerzaGravitacional(const CuerpoEstelar* otro, double& fx, double& fy) const
{
    ...
    // Verificar colisión y evitar división por cero
...
void CuerpoEstelar::setPosicion(double x, double y)
{
    ...
    m_y = y;
    setPos(m_x, m_y);
    // Limpiar trayectoria solo si se cambia posición manualmente
    limpiarTrayectoria();
}
